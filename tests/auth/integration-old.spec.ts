...
test.describe('Authentication Integration Tests', () => {
  ...
  test('form validation and error clearing', async ({ page }) => {
    ...
    await expect(page.locator('text=/required|email|password/i')).not.toBeVisible()
  })
  ...
  test('loading states during authentication', async ({ page }) => {
    ...
    await expect(page.locator('text=/signing|loading|creating/i').or(page.locator('[aria-label*="loading"], .spinner, .animate-spin'))).toBeVisible()
